...
# ── Tool Engine ───────────────────────────────────────────────
# These are the actual analytical functions AURA's AI can call.
# Every number returned here comes from real calculation.
# The AI never invents results — it only explains these results.
...
class AuraTools:
    ...
    def __init__(self, df: pd.DataFrame, filename: str):
        ...
        self.log      = []  # audit trail of every tool call
    ...
    def group_by(self, metric: str, group: str, agg: str = "sum", top_n: int = 10):
        ...
        self._log("group_by", {"metric": metric, "group": group},
                  f"Top: {result_df.iloc[0][group]}={result_df.iloc[0]['value']}")
        return result
    ...
    def compare_periods(self, metric: str, period1: str, period2: str):
        ...
        self._log("compare_periods",
                  {"metric": metric, "p1": period1, "p2": period2},
                  f"{period1}={p1_val} → {period2}={p2_val} ({pct}%)")
        return result

    # ── Tool 5: Monthly trend ─────────────────────────────────
    def monthly_trend(self, metric: str):
        ...
        self._log("monthly_trend", {"metric": metric},
                  f"Best: {best['month']}={best['value']}, Worst: {worst['month']}={worst['value']}")
        return result
    ...
    def detect_anomalies(self, metric: str, threshold: float = 2.0):
        ...
        if std == 0:
            return {"metric": metric, "anomalies": [], "message": "No variance in data"}
        ...
        self._log("detect_anomalies", {"metric": metric},
                  f"Found {len(anomalies)} anomalies in {metric}")
        return result
    ...
    def calculate_correlation(self, col1: str, col2: str):
        ...
        result = {
            ...
            "strength":  strength,
            "direction": direction,
            "interpretation": f"{strength} {direction} correlation between {col1} and {col2}"
        }
        self._log("calculate_correlation", {"col1": col1, "col2": col2},
                  f"r={corr} ({strength} {direction})")
        return result

    # ── Tool 8: Top contributors to a metric ──────────────────
    def top_contributors(self, metric: str, group: str, top_n: int = 5):
        ...
        result["interpretation"] = (
            f"Top {len(result['data'])} {group} groups account for "
            f"{sum(r['pct_of_total'] for r in result['data']):.1f}% of total {metric}"
        )
        self._log("top_contributors", {"metric": metric, "group": group},
                  result["interpretation"])
        return result

    # ── Tool 9: Find available periods ────────────────────────
    def get_available_periods(self):
        ...
        self._log("get_available_periods", {}, f"{len(periods)} periods found")
        return result
